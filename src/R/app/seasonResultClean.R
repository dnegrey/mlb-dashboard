seasonResultClean <- function(seasonResult) {
    # division results
    x <- list(
        AL = seasonResult$standings$AL,
        NL = seasonResult$standings$NL
    )
    x <- lapply(
        X = x,
        FUN = function(x){
    ...
    x <- list(
        AL = x$AL,
        NL = x$NL,
        wonAL = seasonResult$wonAL,
        wonNL = seasonResult$wonNL,
        wonWS = seasonResult$wonWS
    )
    return(x)
}